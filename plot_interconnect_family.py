...
#
# To run script 'python3 plot_interconnect_family.py <interconnect_family_counts_201006_to_202606.csv>'
#
...
def plot_interconnect_families(csv_file):
...
def plot_stacked_area(csv_file):
    df = pd.read_csv(csv_file)
    df['Date'] = pd.to_datetime(df['Date'], format='%Y%m')
...
def plot_percentage_distribution(csv_file):
    df = pd.read_csv(csv_file)
    df['Date'] = pd.to_datetime(df['Date'], format='%Y%m')
...
# Single-column
def plot_column_width(csv_file, year_step=2, legend_loc='upper left',
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Plot interconnect family counts over time (line, stacked area, "
                    "percentage, and single-column versions)"
    )
    parser.add_argument("csv_file", help="Path to input interconnect family counts CSV file")
    args = parser.parse_args()

    plot_interconnect_families(args.csv_file)
    plot_stacked_area(args.csv_file)
    plot_percentage_distribution(args.csv_file)
    plot_column_width(args.csv_file)
