#
# This script plots total core and accelerators core count from 2011-2026 using box plot.
#
# To run script 'python3.6 plot_age.py <accelerator_statistics.csv>'
#
...
def plot_accelerator_statistics(csv_file, output_file="core_statistics.png",
                                figsize=(18, 8), label_fontsize=13, tick_fontsize=11,
                                legend_fontsize=12, box_width=0.35, box_offset=0.2,
                                marker_size=100, year_step=1, legend_loc='upper left',
    ...
    for year, month in unique_year_months:
        ...
        for idx, row in year_month_data.iterrows():
            ...
            if pd.notna(row['Accel_Cores_Median']):
                ...
                accel_positions.append(position_counter - box_offset)  # Offset left
            ...
            if pd.notna(row['Total_Cores_Median']):
                ...
                total_positions.append(position_counter + box_offset)  # Offset right
            
            # Track top system for total cores
            if pd.notna(row['Top_System_Total_Cores']):
                if row['Top_System_Has_Accelerator']:
                    top_with_accel.append((position_counter + box_offset, row['Top_System_Total_Cores']))
                else:
                    top_without_accel.append((position_counter + box_offset, row['Top_System_Total_Cores']))
    ...
    if len(accel_bp_data) > 0:
        bp_accel = ax.bxp(accel_bp_data, positions=accel_positions, widths=box_width,
    ...
    if len(total_bp_data) > 0:
        bp_total = ax.bxp(total_bp_data, positions=total_positions, widths=box_width,
                    patch_artist=True, showfliers=False,
    ...
    if len(top_with_accel) > 0:
        ...
        ax.scatter(pos_with, val_with, marker='o', s=marker_size, color=top_with_accel_color, 
    ...
    if len(top_without_accel) > 0:
        ...
        ax.scatter(pos_without, val_without, marker='s', s=marker_size, color=top_no_accel_color, 
                   alpha=0.9, zorder=6, edgecolors='black', linewidths=1.5)
    ...
    ax.set_ylabel('Core Count', fontsize=label_fontsize, fontweight='bold')
    ax.set_yscale('log')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Plot total and accelerator core count statistics as a box plot, saved as "
                    "both a full-size PNG and a compact 7in PNG"
    )
    parser.add_argument("csv_file", help="Path to input accelerator_statistics CSV file")
    parser.add_argument(
        "--full-output",
        default="core_statistics.png",
        help="Path to full-size output PNG file (default: core_statistics.png)"
    )
    parser.add_argument(
        "--compact-output",
        default="core_statistics_7in.png",
        help="Path to compact 7in output PNG file (default: core_statistics_7in.png)"
    )
    args = parser.parse_args()

    # Full-size
    plot_accelerator_statistics(args.csv_file, args.full_output, figsize=(18, 8))

    # Compact 7in
    plot_accelerator_statistics(
        args.csv_file, args.compact_output, figsize=(18, 7),
        label_fontsize=14, tick_fontsize=13, legend_fontsize=13,
        box_width=0.35, box_offset=0.2, marker_size=80, year_step=1,
        legend_loc='upper left', legend_ncol=3, legend_markerscale=1.0,
        legend_handlelength=1.5, legend_labelspacing=0.4, legend_borderpad=0.4
    )
