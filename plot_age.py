...
def plot_year_field_boxplot(csv_file="year_field_statistics.csv", output_file="year_field_trends.png"):
    ...
    median_color = '#009E73'   # Bluish green (changed from orange)
    top_system_color = '#D55E00' # Vermillion/red-orange (changed for better contrast)
    ...
    # Overlay the top system year as dots (only plot non-None values)
    if any(y is not None for y in top_system_data):
        ...
        ax.scatter(valid_positions, valid_top_years, marker='o', s=80, 
    ...
    ax.set_ylabel('System Installation Year', fontsize=12, fontweight='bold')
