#
# This script graphs architecture, CPU-brand, and GPU-brand trends for academic systems from 2011/11-2026/06.
# The script plots three figures cpu_brand_trend.png, gpu_brand_trend.png, and architecture_trend.png  
#
# To run: 'python3 plot_architecture_brands.py <architecture_brand_statistics_academic.csv>'
#
...
def load_data(csv_file, start=201111, end=202606):
...
def make_xticks(df):
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        if row["Month"] == 6 and row["Year"] not in seen:
            ...
            seen.add(row["Year"])
    # If a year has only a November list (201111), make sure it still shows
    for i, (_, row) in enumerate(df.iterrows()):
...
def main(csv_file, start, end):
    df = load_data(csv_file, start=start, end=end)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Plot architecture, CPU-brand, and GPU-brand trends from a "
                    "architecture_brand_statistics CSV file"
    )
    parser.add_argument("csv_file", help="Path to input architecture_brand_statistics CSV file")
    parser.add_argument(
        "--start", type=int, default=201111,
        help="Only include lists with a date code (YYYYMM) at or after this value (default: 201111)"
    )
    parser.add_argument(
        "--end", type=int, default=202606,
        help="Only include lists with a date code (YYYYMM) at or before this value (default: 202606)"
    )
    args = parser.parse_args()

    main(args.csv_file, args.start, args.end)
