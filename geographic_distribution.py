#
# This script extracts geographic statistics from a directory and outputs the csv with counts per country and continent.
#
# To run script 'python3.6 geographic_distribution.py <lists directory>'
#
...
def extract_geographic_distribution(input_dir, output_file="geographic_distribution.csv"):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract geographic (country/continent) distribution statistics from a directory"
    )
    parser.add_argument("input_dir", help="Path to directory containing CSV files")
    parser.add_argument(
        "-o", "--output",
        default="geographic_distribution.csv",
        help="Path to output CSV file (default: geographic_distribution.csv)"
    )
    args = parser.parse_args()

    df = extract_geographic_distribution(args.input_dir, args.output)
