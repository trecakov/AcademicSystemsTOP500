#
# This script extracts architecture, CPU-brand, and GPU-brand statistics from a directory and outputs a CSV file.
#
# To run: 'python3 architecture_stats.py <lists directory> -o <architecture_brand_statistics_academic.csv>'
#
...
def extract_architecture_brand_statistics(input_dir,
    ...
    for csv_file in sorted(filtered_files):
        try:
            ...
            for _, row in df.iterrows():
                ...
                # GPU brand (only systems with accel)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract architecture, CPU-brand, and GPU-brand statistics from a directory"
    )
    parser.add_argument("input_dir", help="Path to directory containing CSV files")
    parser.add_argument(
        "-o", "--output",
        default="architecture_brand_statistics.csv",
        help="Path to output CSV file (default: architecture_brand_statistics.csv)"
    )
    parser.add_argument(
        "--min-year",
        type=int,
    ...
    df = extract_architecture_brand_statistics(
        args.input_dir, args.output, args.min_year, args.max_year
    )
