#
# This script extracts the power statistics form directory from 06/2008 and outputs the csv file with power stats.
#
# To run script 'python3.6 power_statistics.py <top500 lists directory> -o <output file name>'
#
...
def extract_power_statistics(input_dir, output_file="power_statistics.csv", min_date_code=200806):
    ...
    for csv_file in csv_files:
        ...
        if date_match:
            ...
            if date_code >= min_date_code:
                filtered_files.append(csv_file)
    
    if not filtered_files:
        print(f"No CSV files found from {min_date_code} onwards in {input_dir}")
        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract power consumption statistics from a directory with lists"
    )
    parser.add_argument("input_dir", help="Path to directory containing CSV files")
    parser.add_argument(
        "-o", "--output",
        default="power_statistics.csv",
        help="Path to output CSV file (default: power_statistics.csv)"
    )
    parser.add_argument(
        "--min-date-code",
        type=int,
    ...
    df = extract_power_statistics(args.input_dir, args.output, args.min_date_code)
