#
# This script extracts segment statistics from a directory(TOP500lists/) across years and outputs the csv file.
#
# To run script 'python3.6 segment_distribution_stats.py'
#
...
def extract_segment_distribution(input_dir, output_file="segment_distribution_stats.csv"):
    ...
    for file_path in sorted(all_files):
        try:
            ...
            if segment_column is None:
                print(f"'Segment' column not found, skipping")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract segment distribution statistics from a TOP500lists/ directory"
    )
    parser.add_argument("input_dir", help="Path to TOP500lists/ directory wiht files (CSV/XLSX/XLS)")
    parser.add_argument(
        "-o", "--output",
        default="segment_distribution_stats.csv",
        help="Path to output CSV file (default: segment_distribution_stats.csv)"
    )
    args = parser.parse_args()

    df = extract_segment_distribution(args.input_dir, args.output)
