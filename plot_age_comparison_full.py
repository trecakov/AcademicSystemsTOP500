#
# This script plots age (installation-year) distribution over time using box plots.
# It draws side-by-side box-and-whisker plots for two populations on the same
# graph -- Academic (from year_field_statistics.csv) and Non-Academic
# (from year_field_statistics_non_academic.csv) -- and overlays the #1 system
# installation year of each population.
#
...
def plot_year_field_boxplot(csv_file="year_field_statistics.csv",
                            non_academic_file="year_field_statistics_non_academic.csv",
                            output_file="year_field_trends_full.png"):
    ...
    if os.path.exists(non_academic_file):
        ...
        for _, row in na_df.iterrows():
            ...
            if 'Top_System_Year' in row and pd.notna(row['Top_System_Year']):
                ...
                na_top_val.append(row['Top_System_Year'])
